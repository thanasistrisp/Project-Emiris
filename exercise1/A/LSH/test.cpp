...
int main(void) {
	...
	tuple<vector<int>, vector<double>> ann = lsh.query(q, 10, euclidean_distance);
	vector<int> indices = get<0>(ann);
	vector<double> distances = get<1>(ann);
	cout << indices.size() << endl;
	for (int i = 0; i < (int) indices.size(); i++) {
		cout << p.at(indices.at(i))[0] << " " << p.at(indices.at(i))[1] << " " << p.at(indices.at(i))[2];
		cout << " distance = " << distances.at(i) << endl;
	}
}
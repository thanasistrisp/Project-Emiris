...
The `HashTable` type defined in hypercube.hpp is a hash table that maps a binary string to a list of integers (the indices of the datapoints that have been projected to this binary string). It takes a binary string, hash it to its decimal value and then uses this value as a key to the map. For this reason, it was created a class BinaryString that overloads the equal operator and the hash function to be able to use the std::unordered_map data structure. The max buckets can be about $2^{d'}$ but as most of them will be empty, we dynamically create them.
